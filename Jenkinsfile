...
pipeline {
    environment {
        ...
        GROUPING = "BusinessIndex"

        ORGANIZATION = "ons"
        TEAM = "bi"
        MODULE_NAME = "bi-api-sys-test"
    }
    options {
        skipDefaultCheckout()
    ...
    stages {
        ...
        stage ('Bundle') {
            ...
            steps {
                ...
                dir('conf') {
                    git(url: "$GITLAB_URL/${GROUPING}/${MODULE_NAME}.git", credentialsId: GITLAB_CREDS, branch: "${BRANCH_DEV}")
                }
...
//                    stash name: 'compiled'
            ...
            post {
                ...
                success {
                    colourText("info","Passed all system test for all BI apis.")
                }
                failure {
                    colourText("warn","Failed in system tests for BI apis.")
                }
            }
        }
